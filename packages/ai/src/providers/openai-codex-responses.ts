...
export const streamOpenAICodexResponses: StreamFunction<"openai-codex-responses", OpenAICodexResponsesOptions> = (
	...
	(async () => {
		const output = createInitialCodexOutput(model);

		try {
			const transportState = await prepareCodexTransport(model, context, options, output, stream);

			if (transportState.transport !== "sse" && !transportState.websocketDisabledForSession) {
				if (await tryCodexWebSocketStream(transportState)) return;
			}

			const response = await fetchCodexSseWithRetries(transportState);
			if (!response.body) {
			...
			pushCodexStreamError(stream, output, options, error);
		}
	})();
...
async function prepareCodexTransport(
	model: Model<"openai-codex-responses">,
	context: Context,
	options: OpenAICodexResponsesOptions | undefined,
	output: AssistantMessage,
	stream: AssistantMessageEventStream,
): Promise<CodexTransportState & { websocketDisabledForSession: boolean }> {
	...
	const body = await resolveCodexRequestBody(model, context, options);
...
async function resolveCodexRequestBody(
	model: Model<"openai-codex-responses">,
	context: Context,
	options: OpenAICodexResponsesOptions | undefined,
): Promise<RequestBody> {
...
/**
 * Attempt the Codex WebSocket transport. Returns true when the stream was started
 * and the caller can stop; false when a transport failure should fall through to
 * the SSE path. Re-throws on abort or non-transport errors.
 */
async function tryCodexWebSocketStream(state: CodexTransportState): Promise<boolean> {
	const {
		model,
		options,
		output,
	...
	try {
		...
		appendAssistantMessageDiagnostic(
			...
			createAssistantMessageDiagnostic("provider_transport_failure", error, {
				...
				requestBytes: new TextEncoder().encode(state.bodyJson).byteLength,
			}),
		);
...
async function fetchCodexSseWithRetries(state: CodexTransportState): Promise<Response> {
	...
	for (let attempt = 0; attempt <= maxRetries; attempt++) {
		...
		try {
			response = await fetchCodexSseOnce(state);
			...
			const shouldRetry = attempt < maxRetries && isRetryableError(response.status, errorText);
			if (shouldRetry) {
				await sleep(computeRetryDelay(response, attempt, options), options?.signal);
				continue;
			}
...
/**
 * Single Codex SSE fetch attempt, with the SSE-header-timeout applied via
 * combined abort signals. Resolves with the raw Response; callers decide whether
 * `ok` is good enough or whether to throw.
 */
async function fetchCodexSseOnce(state: CodexTransportState): Promise<Response> {
	const { options, model, sseHeaders, bodyJson } = state;
	const headerTimeout = createSSEHeaderTimeout();
	const combinedSignal = combineAbortSignals([options?.signal, headerTimeout.signal]);
	try {
		return await fetch(resolveCodexUrl(model.baseUrl), {
			method: "POST",
			headers: sseHeaders,
			body: bodyJson,
...
/**
 * Compute the delay (ms) to wait before the next retry attempt. Honors the
 * `Retry-After` header for 429s (capped by `maxRetryDelayMs`); uses exponential
 * backoff when the header is absent.
 */
function computeRetryDelay(response: Response, attempt: number, options?: StreamOptions): number {
	const retryAfterDelayMs = getRetryAfterDelayMs(response.headers);
	if (retryAfterDelayMs === undefined) {
		return BASE_DELAY_MS * 2 ** attempt;
	}
	return response.status === 429 ? capRetryDelayMs(retryAfterDelayMs, options) : retryAfterDelayMs;
}

/**
 * Build the final, friendly error message for a non-retryable Codex response.
 */
async function buildFinalError(errorText: string, response: Response): Promise<Error> {
	const fakeResponse = new Response(errorText, {
		status: response.status,
		statusText: response.statusText,
	});
	const info = await parseErrorResponse(fakeResponse);
	return new Error(info.friendlyMessage || info.message);
}
...
function throwCodexEventError(event: Record<string, unknown>, type: string): never {
	if (type === "error") {
...
async function* mapCodexEvents(events: AsyncIterable<Record<string, unknown>>): AsyncGenerator<ResponseStreamEvent> {
	for await (const event of events) {
		...
		if (type === "error" || type === "response.failed") {
			throwCodexEventError(event, type);
		}

		if (type === "response.done" || type === "response.completed" || type === "response.incomplete") {
			yield mapCodexResponseCompletedEvent(event);
			return;
		}
...
async function connectWebSocket(
	...
	return await waitForWebSocketConnection(socket, signal, connectTimeoutMs);
}

interface WebSocketConnectHandlers {
	socket: WebSocketLike;
	signal?: AbortSignal;
...
function waitForWebSocketConnection(socket: WebSocketLike, signal: AbortSignal | undefined, connectTimeoutMs: number) {
	return new Promise<WebSocketLike>((resolve, reject) => {
		const ctx: WebSocketConnectHandlers = {
			socket,
			signal,
			connectTimeoutMs,
			resolve,
		...
		const controller = createWebSocketConnectController(ctx);
		socket.addEventListener("open", controller.onOpen);
		socket.addEventListener("error", controller.onError);
		socket.addEventListener("close", controller.onClose);
		signal?.addEventListener("abort", controller.onAbort);
		controller.start();
	});
}

function createWebSocketConnectController(ctx: WebSocketConnectHandlers): {
	start: () => void;
	...
	const cleanup = () => {
		...
		ctx.socket.removeEventListener("open", onOpen);
		ctx.socket.removeEventListener("error", onError);
		ctx.socket.removeEventListener("close", onClose);
	...
	const fail = (error: Error, closeReason?: string) => {
		...
		cleanup();
		if (closeReason) {
			closeWebSocketSilently(ctx.socket, 1000, closeReason);
		}
		ctx.reject(error);
	};
	...
	return {
		start: () => {
			if (ctx.connectTimeoutMs > 0) {
				timeout = setTimeout(() => {
					fail(new Error(`WebSocket connect timeout after ${ctx.connectTimeoutMs}ms`), "connect_timeout");
...
function extractWebSocketCloseError(event: unknown): Error {
	...
	const code = extractOptionalProperty(event, "code");
	const reason = extractOptionalProperty(event, "reason");
	const wasClean = extractOptionalProperty(event, "wasClean");
	const codeText = typeof code === "number" ? ` ${code}` : "";
	const hasReason = typeof reason === "string" && reason.length > 0;
	let reasonText = hasReason ? ` ${reason}` : "";
...
function extractOptionalProperty(obj: object, key: string): unknown {
...
async function* parseWebSocket(
	...
	const state: ParseWebSocketState = {
		queue,
		get done() {
			return done;
		},
		...
		set failed(value: Error | null) {
			failed = value;
		},
		get sawCompletion() {
			return sawCompletion;
		},
		set sawCompletion(value: boolean) {
			sawCompletion = value;
		},
		wake,
	};

	const { onMessage, onError, onClose, onAbort } = createParseWebSocketListeners(state);
...
function handleParseWebSocketMessage(state: ParseWebSocketState, event: unknown): void {
	void (async () => {
		...
		try {
			...
			state.failed = new CodexProtocolError(`Invalid Codex WebSocket JSON: ${formatThrownValue(cause)}`, {
				cause,
				payload: text,
			});
			state.done = true;
...
function handleParseWebSocketClose(state: ParseWebSocketState, event: unknown): void {
...
function createParseWebSocketListeners(state: ParseWebSocketState): {
	...
	return {
		onMessage: (event) => handleParseWebSocketMessage(state, event),
		onError: (event) => {
			state.failed = extractWebSocketError(event);
			state.done = true;
			state.wake();
		},
		onClose: (event) => handleParseWebSocketClose(state, event),
		onAbort: () => {
...
async function processWebSocketStream(
	...
	if (options?.sessionId) {
		recordWebSocketRequestStats(options.sessionId, {
			reused,
			useCachedContext,
			requestBody,
		});
	...
	try {
		...
		if (options?.signal?.aborted) {
			...
			entry.continuation = createWebSocketContinuationState(model, fullBody, output);
		}
	} catch (error) {
		if (entry) {
			entry.continuation = undefined;
...
interface WebSocketRequestStatsInput {
	reused: boolean;
	useCachedContext: boolean;
	requestBody: RequestBody;
}

function recordWebSocketRequestStats(sessionId: string, input: WebSocketRequestStatsInput): void {
	const stats = getOrCreateWebSocketDebugStats(sessionId);
	const { reused, useCachedContext, requestBody } = input;
	stats.requests++;
...
function createWebSocketContinuationState(
	model: Model<"openai-codex-responses">,
