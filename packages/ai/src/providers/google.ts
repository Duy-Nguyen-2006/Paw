...
import { convertMessages, convertTools, mapToolChoice } from "./google-shared.ts";
import {
	assertGoogleStreamCompleted,
	createGoogleStreamProcessState,
	finalizeOpenGoogleStreamBlock,
	handleGoogleStreamError,
	processGoogleStreamChunk,
} from "./google-stream-helpers.ts";
...
export const streamGoogle: StreamFunction<"google-generative-ai", GoogleOptions> = (
	...
	(async () => {
		...
		try {
			...
			const processState = createGoogleStreamProcessState();
			for await (const chunk of googleStream) {
				processGoogleStreamChunk(stream, output, processState, model, chunk);
			}
