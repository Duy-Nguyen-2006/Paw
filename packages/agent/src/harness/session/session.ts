...
function appendEntryToMessages(entry: SessionTreeEntry, messages: AgentMessage[]): void {
...
function extractSessionMetadata(pathEntries: SessionTreeEntry[]): {
...
export function buildSessionContext(pathEntries: SessionTreeEntry[]): SessionContext {
	const { thinkingLevel, model, activeToolNames, compaction } = extractSessionMetadata(pathEntries);
	...
	if (compaction) {
		...
		for (let i = 0; i < compactionIdx; i++) {
			...
			if (foundFirstKept) appendEntryToMessages(entry, messages);
		}
		for (let i = compactionIdx + 1; i < pathEntries.length; i++) {
			appendEntryToMessages(pathEntries[i]!, messages);
		}
	} else {
		for (const entry of pathEntries) {
			appendEntryToMessages(entry, messages);
		}
	}
