/**
 * Resource path formatting and diagnostics (reduces InteractiveMode / showLoadedResources complexity).
 */
...
export function resolveLocalSourceInfo(scope: string): { label: string; scopeLabel?: string; color: "muted" } {
...
export function formatExtensionDisplayPath(resourcePath: string): string {
	let result = formatDisplayPath(resourcePath);
...
export type ScopeResourceGroup = {
	scope: "user" | "project" | "path";
	paths: Array<{ path: string; sourceInfo?: SourceInfo }>;
	packages: Map<string, Array<{ path: string; sourceInfo?: SourceInfo }>>;
};

export function buildScopeGroups(items: Array<{ path: string; sourceInfo?: SourceInfo }>): ScopeResourceGroup[] {
	const groups: Record<"user" | "project" | "path", ScopeResourceGroup> = {
...
export function formatScopeGroups(
	...
	for (const group of groups) {
		...
		for (const [source, groupItems] of sortedPackages) {
			lines.push(`    ${theme.fg("mdLink", source)}`);
			const sortedPackagePaths = [...groupItems].sort((a, b) => a.path.localeCompare(b.path));
