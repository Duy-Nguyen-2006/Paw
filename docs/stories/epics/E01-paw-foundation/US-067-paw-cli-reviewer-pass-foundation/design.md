...
`runPawCompleteReviewerCommand` parses bounded CLI args, then `createPawCompleteReviewerCommandResult`
checks `.paw` and session state, reads and parses `--output-file` with `parsePawSubAgentOutputJson`,
acquires the session lock, calls `completePawReviewerPass`, and releases an owned lock before
returning.

## Lock Semantics

- Parse reviewer output before acquiring lock when project/session exist; invalid or missing output
  files return without lock acquisition.
- Acquire lock before calling `completePawReviewerPass`, matching other bounded CLI commands.
- `locked` at acquire time: do not release a live foreign lock.
- Completed, invalid_state, no_selected_slice, invalid_reviewer_output, reviewer_not_passed,
