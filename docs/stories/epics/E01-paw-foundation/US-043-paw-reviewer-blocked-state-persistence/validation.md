...
US-043 is complete when the core helper persists blocked reviewer output for the
...
- Deterministic session ids, lock owners, selected slice ids, reviewer outputs,
  and blocked reasons.
...
```text
cd packages/coding-agent && node ../../node_modules/vitest/dist/cli.js --run test/paw-reviewer-blocked-result.test.ts test/paw-reviewer-result.test.ts test/paw-state.test.ts test/paw-task-session.test.ts
...
- Focused reviewer-blocked-result and adjacent Paw tests passed:
  `cd packages/coding-agent && node ../../node_modules/vitest/dist/cli.js --run test/paw-reviewer-blocked-result.test.ts test/paw-reviewer-result.test.ts test/paw-state.test.ts test/paw-task-session.test.ts`.
