...
Persist the blocked reviewer boundary for a selected Paw slice. The helper
requires a live current session lock, validates blocked reviewer output, maps
the blocked reason code to the matching `BLOCKED_*` state, and writes the
blocked session state.

## Scope

- Add a core reviewer-blocked-result helper.
- Support reviewer `blocked` and `needs_user_decision` outputs.
...
- Leave reviewer fail/retry policy, verifier blocked handling, and CLI resume
  routing to later slices.

## Acceptance Criteria

- Blocked reviewer output in `REVIEWING` persists the matching `BLOCKED_*`
  state.
