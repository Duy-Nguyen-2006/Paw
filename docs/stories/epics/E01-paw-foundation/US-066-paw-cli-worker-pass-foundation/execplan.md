...
1. Implement `worker-result-command.ts` mirroring `slice-implementation-command.ts` and
   `slice-checkpoint-command.ts` lock-before-domain-call and release-after-outcome flow.
2. Wire `handlePawCommand` and root `printPawHelp` in `init-command.ts`.
3. Export public types and functions from `index.ts`.
4. Add `paw-worker-result-command.test.ts` with parser, integration, routing, and `main` coverage.
