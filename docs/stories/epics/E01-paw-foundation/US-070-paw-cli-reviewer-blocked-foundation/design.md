...
acquires the session lock, calls `blockPawReviewerResult`, and releases an owned lock before returning.
...
- Acquire lock before calling `blockPawReviewerResult`, matching other bounded CLI commands.
- `locked` at acquire time: do not release a live foreign lock.
- Blocked, invalid_state, no_selected_slice, invalid_reviewer_output, reviewer_not_blocked,
