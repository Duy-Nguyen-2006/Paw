...
Add `paw block-reviewer <session-id> --output-file <path>` as a bounded CLI foundation that reads
reviewer sub-agent output JSON, acquires the session lock, calls `blockPawReviewerResult`, and releases
an owned lock before returning.

## Scope

- Add `reviewer-blocked-command.ts` with parser, result builder, formatting, and `runPawBlockReviewerCommand`.
- Route `paw block-reviewer` through `handlePawCommand` before the normal agent runtime.
- Validate reviewer output JSON before lock acquisition when project and session exist.
- Report structured outcomes including `blocked`, `invalid_output_file`, `missing_output_file`,
  ...
  `invalid_reviewer_output`, `reviewer_not_blocked`, `invalid_blocked_reason`, `invalid_transition`,
  `not_locked`, and `locked_by_other`.
- Export block-reviewer helpers from `packages/coding-agent/src/paw/index.ts`.
- Add focused tests and Harness story documentation.

## Acceptance Criteria

- `REVIEWING` sessions with blocked or `needs_user_decision` reviewer output advance to matching
  `BLOCKED_*` states.
...
- Owned locks acquired by the command are released for blocked, invalid_state, no_selected_slice,
  invalid_reviewer_output, reviewer_not_blocked, invalid_blocked_reason, invalid_transition, not_locked,
